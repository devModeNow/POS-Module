...
export class DashboardService {
  ...
  async getOrgDashboard(period: 'daily' | 'weekly' | 'monthly' = 'daily'): Promise<any> {
    const response = await apiClient.get('/dashboard/org', { params: { period } });
    return response.data;
  }

  async addExpense(dto: { description: string; amount: number; category?: string; expenseDate?: string }): Promise<any> {
    const response = await apiClient.post('/dashboard/org/expense', dto);
    return response.data;
  }

  async addDeposit(dto: { description: string; amount: number; source?: string }): Promise<any> {
    const response = await apiClient.post('/dashboard/org/deposit', dto);
    return response.data;
  }
}
